    ...
    $.widget('mock.rawedit', _view, {
        ...
        _createBlankElem: function() {
            ...
            h.push('<tr>' +
                ...
                '<span class="errorinfo" for="upload-pk" style="display:none;"></span>' +
                ...
                '<span class="errorinfo" for="upload-vote-box" style="display:none;"></span>' +
                '<div class="upload-vote-box" id="upload-vote-box">' +
                '<div class="upload-vote-item">' +
                '<div class="mock-input-box">' +
                '<span class="errorinfo" for="" style="display:none;"></span>' +
                '<input type="text" maxlength="20" class="form-control upload-vote-title">' +
                '<span class="mock-input-tip">最多20字符</span>' +
                '</div>' +
                '<div class="mock-btn mock-btn-red upload-vote-item-addimg">+ 添加图片</div><button class="btn btn-default upload-vote-item-remove">- 移除观点</button>' +
                ...
                '<button class="btn btn-default upload-vote-item-clearimg">- 移除图片</button>' +
        ...
        _createRawElem: function(item) {
            var options = this.options,
                h = [],
                ext;
            options.lastext = item.ext.replace(/,"id":\d*/ig, '');
            h.push(
            ...
            if ((item.type == '2') && (!!item.ext) && (!_.isEmpty(JSON.parse(item.ext)))) {
                ...
                _.each(ext, function(vote, index) {
                    h.push(
                        ...
                        '<div class="mock-input-box">' +
                        '<span class="errorinfo" for="" style="display:none;"></span>' +
                        '<input type="text" maxlength="20" class="form-control upload-vote-title" value="' + vote.title + '">' +
                        '<span class="mock-input-tip">最多20字符</span>' +
                        '</div>' +
                        '<div class="mock-btn mock-btn-red upload-vote-item-addimg">+ 添加图片</div><button class="btn btn-default upload-vote-item-remove">- 移除观点</button>');
                    if (!!vote.img) {
            ...
            $('#upload-content').Editor();
            $('#upload-content').Editor('setText', _.unescape(item.content));
            $('#upload-uptime').datetimepicker({
                format: 'Y-m-d H:i',
                lang: 'ch',
                yearStart: 2015,
        ...
        _bindEvents: function() {
            this._on(this.element, {
                ...
                'click button.upload-vote-item-clearimg': this._clearImg,
                'click #upload-vote-additem': this._addVoteItem,
                'click button.upload-vote-item-remove': this._removeVoteItem,
        ...
        _clearImg: function(event) {
            ...
            $imgbox.find('input').val('').trigger('change');
            $imgbox.children('div.upload-img-preivew').empty().addClass('hide');
        ...
        _addVoteItem: function() {
            ...
            h.push('<div class="upload-vote-item">' +
                ...
                '<input type="text" maxlength="20" class="form-control upload-vote-title">' +
                '<span class="mock-input-tip">最多20字符</span>' +
                '</div><div class="mock-btn mock-btn-red upload-vote-item-addimg">+ 添加图片</div><button class="btn btn-default upload-vote-item-remove">- 移除观点</button>' +
                '<div class="upload-img-box-wrapper hide">' +
                '<div class="bg-warning">图片尺寸要求：宽度200px，高度200px。</div>' +
                '<div class="upload-img-box">' +
                '<div class="upload-img">' +
        ...
        _removeVoteItem: function(event) {
            $(event.target).closest('div.upload-vote-item').remove();
        },
        ...
        _submitData: function(event) {
            ...
            switch (type) {
                ...
                case '2':
                    ...
                    if ((ext.length > 2) && (ext.length < 7)) {
                        ...
                        isValidate = false;
                        $('span[for=upload-vote-box]').html('内容类型 为 投票 的观点个数最少3个，最多6个，且观点内容不为空。').show();
                        return false;
                    }
                    break;
            ...
            if (isValidate) {
                var rawdata = {
                    ...
                    sendData = {};
                if (options.lastext != JSON.stringify(ext)) {
                    $.extend(rawdata, {
                        ext: ext
                    });
                }
